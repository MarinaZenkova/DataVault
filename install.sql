...
---------------------------------------------
-- СОЗДАНИЕ СЛОЯ DDS - DATA VAULT 2.0
---------------------------------------------
CREATE SCHEMA dds;


CREATE TABLE dds.h_order (
    HubSalesOrderHashKey uuid NOT NULL,
    SalesOrderID INT NOT NULL,
...
INSERT INTO dds.h_order (HubSalesOrderHashKey, SalesOrderID, LoadDateTime, RecordSource)
SELECT 
    CAST(MD5(CAST(SalesOrderID AS VARCHAR(50))) AS UUID) AS HubSalesOrderHashKey,
    SalesOrderID,
    orderdate,
    'AdventureWorks2014' AS RecordSource
FROM Sales.SalesOrderHeader;



CREATE TABLE dds.h_product (
    HubProductHashKey UUID,
    ProductID INT NOT NULL,
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50)
);
...
INSERT INTO dds.h_product (HubProductHashKey, ProductID, LoadDateTime, RecordSource)
SELECT 
    CAST(MD5(CAST(ProductID AS VARCHAR(50))) AS UUID) AS HubProductHashKey,
    ProductID,
    sellstartdate,
    'AdventureWorks2014' AS RecordSource
...
CREATE TABLE dds.h_order_detail (
    HubOrderDetailHashKey UUID PRIMARY KEY,
...
INSERT INTO dds.h_order_detail (HubOrderDetailHashKey, SalesOrderDetailID, SalesOrderID, LoadDateTime, RecordSource)
SELECT 
    MD5(CONCAT_WS('||', 
        COALESCE(NULLIF(CAST(SalesOrderDetailID AS VARCHAR), ''), '^^'),
        COALESCE(NULLIF(CAST(SalesOrderID AS VARCHAR), ''), '^^')
    ))::UUID AS HubOrderDetailHashKey,
    SalesOrderDetailID,
    SalesOrderID,
    modifieddate,
    'AdventureWorks2014' AS RecordSource
...
CREATE TABLE dds.h_product_category (
    HubProductCategoryHashKey UUID PRIMARY KEY,
    ProductCategoryID INT NOT NULL,
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50)
);

INSERT INTO dds.h_product_category (
    HubProductCategoryHashKey,
    ProductCategoryID,
    LoadDateTime,
    RecordSource)
SELECT 
    CAST(MD5(CAST(ProductCategoryID AS VARCHAR(50))) AS UUID) AS HubProductCategoryHashKey,
    ProductCategoryID,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
...
CREATE TABLE dds.h_product_subcategory (
    HubProductSubCategoryHashKey UUID PRIMARY KEY,
...
INSERT INTO dds.h_product_subcategory (
    HubProductSubCategoryHashKey,
    ProductSubCategoryID,
    ProductCategoryID,
    LoadDateTime,
    RecordSource)
SELECT 
    CAST(MD5(CAST(ProductSubCategoryID AS VARCHAR(50))) AS UUID) AS HubProductSubCategoryHashKey,
    ProductSubCategoryID,
    ProductCategoryID,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
...
CREATE TABLE dds.l_sales_order_order_detail_product (
    HubSalesOrderHashKey UUID NOT NULL,
    HubOrderDetailHashKey UUID NOT NULL,
    HubProductHashKey UUID NOT NULL,
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50)
);

INSERT INTO dds.l_sales_order_order_detail_product (HubSalesOrderHashKey, HubOrderDetailHashKey, HubProductHashKey, LoadDateTime, RecordSource)
SELECT 
    o.HubSalesOrderHashKey,
    od.HubOrderDetailHashKey,
    p.HubProductHashKey,
     modifieddate,
    'AdventureWorks2014' AS RecordSource
FROM Sales.SalesOrderDetail sod
JOIN dds.h_order o ON sod.SalesOrderID = o.SalesOrderID
JOIN dds.h_order_detail od ON sod.SalesOrderDetailID = od.SalesOrderDetailID
JOIN dds.h_product p ON sod.ProductID = p.ProductID;




CREATE TABLE dds.l_product_product_subcategory (
    HubProductHashKey UUID NOT NULL,
    HubProductSubCategoryHashKey UUID NOT NULL,
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50)
);

INSERT INTO dds.l_product_product_subcategory (
    HubProductHashKey, 
    HubProductSubCategoryHashKey, 
    LoadDateTime, 
    RecordSource)
SELECT 
    p.HubProductHashKey,
    ps.HubProductSubCategoryHashKey,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM Production.Product pr
JOIN dds.h_product p ON pr.ProductID = p.ProductID
JOIN dds.h_product_subcategory ps ON pr.ProductSubcategoryID = ps.ProductSubCategoryID;

CREATE TABLE dds.l_product_subcategory_product_category (
    HubProductSubCategoryHashKey UUID NOT NULL,
    HubProductCategoryHashKey UUID NOT NULL,
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50)
);

INSERT INTO dds.l_product_subcategory_product_category (
    HubProductSubCategoryHashKey, 
    HubProductCategoryHashKey, 
    LoadDateTime, 
    RecordSource)
SELECT 
    ps.HubProductSubCategoryHashKey,
    pc.HubProductCategoryHashKey,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM Production.ProductSubcategory psc
JOIN dds.h_product_subcategory ps ON psc.ProductSubCategoryID = ps.ProductSubCategoryID
JOIN dds.h_product_category pc ON psc.ProductCategoryID = pc.ProductCategoryID;


CREATE TABLE dds.s_order (
    HubSalesOrderHashKey UUID NOT NULL,
    SalesOrderID INT NOT NULL,
    OrderDate TIMESTAMP NOT NULL,
    TotalAmount DECIMAL(18, 2),
    ExpenseFreight DECIMAL(18, 2),
...
INSERT INTO dds.s_order (HubSalesOrderHashKey,SalesOrderID,  OrderDate, TotalAmount,ExpenseFreight, TotalTax,  SubTotal, ValidFrom,  ValidTo,  LoadDateTime, RecordSource)
SELECT 
    o.HubSalesOrderHashKey,
    o.salesorderid,
    soh.OrderDate,
    ...
    soh.subtotal,
    soh.orderdate,
    '9999-12-31 00:00:00.000'::timestamp,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM Sales.SalesOrderHeader soh
JOIN dds.h_order o ON soh.SalesOrderID = o.SalesOrderID;



CREATE TABLE dds.s_product (
    HubProductHashKey UUID NOT NULL,
    ProductID INT NOT NULL,
    Name VARCHAR(50),
...
SELECT 
    ...
    pr.ListPrice,
    pr.Size,
    modifieddate AS ValidFrom,
    '9999-12-31 23:59:59'::timestamp AS ValidTo,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM Production.Product pr
JOIN dds.h_product p ON pr.ProductID = p.ProductID;
...
CREATE TABLE dds.s_order_detail (
    HubOrderDetailHashKey UUID NOT NULL,
    SalesOrderDetailID INT NOT NULL,
    SalesOrderID INT NOT NULL,
    OrderQty INT,
...
INSERT INTO dds.s_order_detail (
    HubOrderDetailHashKey, 
    SalesOrderDetailID, 
    SalesOrderID, 
    OrderQty, 
    UnitPrice, 
...
CREATE TABLE dds.s_product_category (
    HubProductCategoryHashKey UUID NOT NULL,
    ProductCategoryID INT NOT NULL,
    Name VARCHAR(50),
    ValidFrom TIMESTAMP NOT NULL,
    ...
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50),
    PRIMARY KEY (HubProductCategoryHashKey)
);


INSERT INTO dds.s_product_category (
    HubProductCategoryHashKey, 
    ProductCategoryID, 
    Name, 
    ValidFrom, 
...
JOIN dds.h_product_category hpc ON pc.ProductCategoryID = hpc.ProductCategoryID;



CREATE TABLE dds.s_product_subcategory (
    HubProductSubCategoryHashKey UUID NOT NULL,
    ProductSubCategoryID INT NOT NULL,
    ProductCategoryID INT NOT NULL,
    Name VARCHAR(50),
    ...
    LoadDateTime TIMESTAMP NOT NULL,
    RecordSource VARCHAR(50),
    PRIMARY KEY (HubProductSubCategoryHashKey)
);

INSERT INTO dds.s_product_subcategory (
    HubProductSubCategoryHashKey,
    ProductSubCategoryID,
    ProductCategoryID,
    Name,
    ...
    RecordSource)
SELECT 
    ps_h.HubProductSubCategoryHashKey,
...
JOIN dds.h_product_subcategory ps_h 
ON ps.ProductSubcategoryID = ps_h.ProductSubCategoryID;

---------------------------------------------
-- COЗДАНИЕ СЛОЯ DATA MART - STAR SCHEMA
---------------------------------------------
...
CREATE TABLE data_mart.fct_order  (
    HubSalesOrderHashKey UUID NOT NULL,
    OrderDate TIMESTAMP NOT NULL,
    TotalAmount DECIMAL(18, 2),
    ExpensesFreight DECIMAL(18, 2),
    Tax DECIMAL(18, 2),
    CostOfGoodsSold DECIMAL(18, 2),
    NetProfit DECIMAL(18, 2),
    PRIMARY KEY (HubSalesOrderHashKey, OrderDate)
);

INSERT INTO data_mart.fct_order (
    HubSalesOrderHashKey,
    OrderDate,
    TotalAmount,
    ExpensesFreight,
    Tax,
...
SELECT 
    ...
    o.ExpenseFreight,
    o.TotalTax,
    COALESCE(SUM(pd.StandardCost * sod.OrderQty), 0) AS CostOfGoodsSold,
    o.TotalAmount - o.ExpenseFreight - o.TotalTax - COALESCE(SUM(pd.StandardCost * sod.OrderQty), 0) AS NetProfit
FROM dds.s_order o
JOIN dds.l_sales_order_order_detail_product l ON o.HubSalesOrderHashKey = l.HubSalesOrderHashKey
JOIN dds.s_order_detail sod ON l.HubOrderDetailHashKey = sod.HubOrderDetailHashKey
JOIN dds.s_product pd ON l.HubProductHashKey = pd.HubProductHashKey
WHERE o.IsCurrent = TRUE AND sod.IsCurrent = TRUE AND pd.IsCurrent = TRUE
GROUP BY o.HubSalesOrderHashKey, o.OrderDate, o.TotalAmount, o.ExpenseFreight, o.TotalTax;



CREATE TABLE data_mart.fct_order_detail (
    HubOrderDetailHashKey UUID NOT NULL,
    HubSalesOrderHashKey UUID NOT NULL,
    ProductID INT NOT NULL,
    ProductCategoryID INT NOT NULL,
...
INSERT INTO data_mart.fct_order_detail (
    HubOrderDetailHashKey,
    HubSalesOrderHashKey,
    ProductID,
    ProductCategoryID,
...
SELECT
    ...
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM dds.s_order_detail sod
JOIN dds.h_order_detail od ON sod.SalesOrderDetailID = od.SalesOrderDetailID
JOIN dds.l_sales_order_order_detail_product l ON od.HubOrderDetailHashKey = l.HubOrderDetailHashKey
JOIN dds.h_product p ON l.HubProductHashKey = p.HubProductHashKey
JOIN dds.l_product_product_subcategory lps ON p.HubProductHashKey = lps.HubProductHashKey
JOIN dds.h_product_subcategory ps ON lps.HubProductSubCategoryHashKey = ps.HubProductSubCategoryHashKey
JOIN dds.h_product_category pc ON ps.ProductCategoryID = pc.ProductCategoryID
WHERE sod.IsCurrent = TRUE;



CREATE TABLE data_mart.dim_product_with_categories (
    ProductID INT NOT NULL,
...
INSERT INTO data_mart.dim_product_with_categories (
    ProductID, 
    ProductName, 
    CategoryID, 
    CategoryName, 
...
FROM Production.Product pr
JOIN dds.h_product p ON pr.ProductID = p.ProductID
JOIN dds.l_product_product_subcategory lpps ON p.HubProductHashKey = lpps.HubProductHashKey
JOIN dds.h_product_subcategory ps ON lpps.HubProductSubCategoryHashKey = ps.HubProductSubCategoryHashKey
JOIN dds.s_product_subcategory psc ON ps.ProductSubCategoryID = psc.ProductSubCategoryID
JOIN dds.s_product_category pc ON psc.ProductCategoryID = pc.ProductCategoryID
WHERE  psc.IsCurrent = TRUE AND pc.IsCurrent = TRUE;
...
WITH RECURSIVE DateRange AS (
    SELECT '2001-01-01'::DATE AS DateKey
    UNION ALL
    SELECT (DateKey + INTERVAL '1 day')::DATE
    FROM DateRange
    WHERE (DateKey + INTERVAL '1 day') <= '2024-12-31'
)
...
SELECT
    ...
    EXTRACT(DAY FROM DateKey) AS Day,
    TO_CHAR(DateKey, 'Month') AS MonthName,
    CONCAT('Q', EXTRACT(QUARTER FROM DateKey)) AS QuarterName,
    NOW() AS LoadDateTime,
    'AdventureWorks2014' AS RecordSource
FROM DateRange;

-------------------------------------------------------
-- CОЗДАНИЕ СЛОЯ АНАЛИТИЧЕСКИХ ВИТРИН INFORMATION MART
-------------------------------------------------------

CREATE SCHEMA information_mart;


CREATE TABLE information_mart.aggregated_finance_metrics (
    DateKey DATE,
    Year INT,
    Quarter INT,
    Month INT,
...
INSERT INTO information_mart.aggregated_finance_metrics
SELECT 
    d.DateKey,
    d.Year,
    d.Quarter,
    d.Month,
    SUM(fo.TotalAmount) AS TotalIncome,
    SUM(fo.ExpensesFreight) + SUM(fo.Tax) + SUM(fo.CostOfGoodsSold) AS TotalExpenses,
    SUM(fo.Tax) AS Taxes,
    SUM(fo.CostOfGoodsSold) AS CostOfGoodsSold,
    SUM(fo.NetProfit) AS NetProfit
FROM data_mart.dim_date d
JOIN data_mart.fct_order fo ON d.DateKey = DATE_TRUNC('day', fo.OrderDate)
GROUP BY d.DateKey, d.Year, d.Quarter, d.Month;



CREATE TABLE information_mart.total_income_growth (
    DateKey DATE,
    Year INT,
    Month INT,
    TotalIncome DECIMAL(18, 2),
    PreviousMonthIncome DECIMAL(18, 2),
    IncomeGrowth DECIMAL(18, 2)
);


INSERT INTO information_mart.total_income_growth
WITH MonthlyIncome AS (
    SELECT
        DATE_TRUNC('month', d.DateKey) AS DateKey,
        d.Year,
        d.Month,
        SUM(fo.TotalAmount) AS TotalIncome
    FROM data_mart.dim_date d
    JOIN data_mart.fct_order fo ON d.DateKey = DATE_TRUNC('day', fo.OrderDate)
    GROUP BY DATE_TRUNC('month', d.DateKey), d.Year, d.Month
)
SELECT
    mi.DateKey,
    mi.Year,
    mi.Month,
    mi.TotalIncome,
    LAG(mi.TotalIncome) OVER (ORDER BY mi.DateKey) AS PreviousMonthIncome,
    (mi.TotalIncome - LAG(mi.TotalIncome) OVER (ORDER BY mi.DateKey)) / NULLIF(LAG(mi.TotalIncome) OVER (ORDER BY mi.DateKey), 0) AS IncomeGrowth
...
CREATE TABLE information_mart.average_order_value_growth (
    DateKey DATE,
    Year INT,
    Month INT,
    AverageOrderValue DECIMAL(18, 2),
    PreviousMonthAOV DECIMAL(18, 2),
    AOVGrowth DECIMAL(18, 2)
);

INSERT INTO information_mart.average_order_value_growth
WITH MonthlyAOV AS (
    SELECT
        DATE_TRUNC('month', d.DateKey) AS DateKey,
        d.Year,
...
SELECT
    ...
    ma.Year,
    ma.Month,
    ma.AverageOrderValue,
    LAG(ma.AverageOrderValue) OVER (ORDER BY ma.DateKey) AS PreviousMonthAOV,
    (ma.AverageOrderValue - LAG(ma.AverageOrderValue) OVER (ORDER BY ma.DateKey)) / NULLIF(LAG(ma.AverageOrderValue) OVER (ORDER BY ma.DateKey), 0) AS AOVGrowth
FROM MonthlyAOV ma;



CREATE TABLE information_mart.sales_per_category_subcategory_month (
    DateKey DATE,
    Year INT,
    Month INT,
    CategoryID INT,
...
INSERT INTO information_mart.sales_per_category_subcategory_month
SELECT 
    DATE_TRUNC('month', d.DateKey) AS DateKey,
    d.Year,
    d.Month,
...
FROM data_mart.dim_date d
JOIN data_mart.fct_order fo ON d.DateKey = DATE_TRUNC('day', fo.OrderDate)
JOIN data_mart.fct_order_detail fod ON fo.HubSalesOrderHashKey = fod.HubSalesOrderHashKey
JOIN data_mart.dim_product_with_categories pc ON fod.ProductID = pc.ProductID
JOIN data_mart.dim_product_with_categories psc ON pc.SubCategoryID = psc.SubCategoryID
GROUP BY DATE_TRUNC('month', d.DateKey), d.Year, d.Month, pc.CategoryID, pc.CategoryName, psc.SubCategoryID, psc.SubCategoryName;
